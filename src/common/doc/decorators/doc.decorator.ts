...
import { ENUM_POLICY_STATUS_CODE_ERROR } from 'src/common/policy/enums/policy.status-code.enum';
import { ENUM_AUTH_STATUS_CODE_ERROR } from 'src/common/auth/enums/auth.status-code.enum';
...
import { ResponsePagingDto } from 'src/common/response/decorators/response.paging.dto';
import { ENUM_APP_STATUS_CODE_ERROR } from 'src/modules/app/enums/app.status-code.enum';
...
export function DocRequest(options?: IDocRequestOptions) {
  ...
  } else if (options?.bodyType === ENUM_DOC_REQUEST_BODY_TYPE.FORM_URLENCODED) {
